...
def find_videos( name, program_id, topic, page ):
	...
	for results in data:
		...
		if results['mediafiles'][0]['video_data_url'] != None:
			...
			mode = '7'
		if topic == 'False':
			play_video( results['title'], url, thumb, results['long_description'], name, None, backup_url )
			return
		if results['mediafiles'][0]['length_mseconds'] != 0:
			listitem = xbmcgui.ListItem( label = results['title'], iconImage = thumb, thumbnailImage = thumb )
			listitem.setInfo( type="Video", infoLabels={ "Title": results['title'], "Director": "PBS", "Studio": name, "Plot": results['long_description'].encode('utf-8'), "Duration": str(datetime.timedelta(milliseconds=int(results['mediafiles'][0]['length_mseconds']))), "Aired": results['airdate'].rsplit(' ')[0] } )
			u = sys.argv[0] + "?mode=" + mode + "&name=" + urllib.quote_plus( results['title'] ) + "&url=" + urllib.quote_plus( url ) + "&thumb=" + urllib.quote_plus( thumb ) + "&plot=" + urllib.quote_plus( results['long_description'].encode('utf-8') ) + "&studio=" + urllib.quote_plus( name ) + "&backup_url=" + urllib.quote_plus( backup_url )
			ok = xbmcplugin.addDirectoryItem( handle = int( sys.argv[1] ), url = u, listitem = listitem, isFolder = False )
	if ( len(data) ) == 200:
		listitem = xbmcgui.ListItem( label = '[Next Page (' + str( int( page ) + 2 ) + ')]' , iconImage = next_thumb, thumbnailImage = next_thumb )
...
def play_video( name, url, thumb, plot, studio, starttime, backup_url ):
	...
	if base == 'http://ad.doubleclick.net/adx/':
		...
		play_mp4( name, base+src, thumb, plot, studio, starttime )
		return
	elif base.find('/vod') != -1:
		rtmp_url = base
		playpath = "mp4:" + src
	else:
		rtmp_url = base + "mp4:" + src.replace('mp4:','')
